...
const BillableTimeEntriesContent = () => {
  ...
  const { getAllJobCategories, getAllTimeEntries,getAllBillableTimeEntries, initialState } =
  ...
  useEffect(() => {
    // filter by status
    const filterByStatus = initialState?.allBillableTimeEntries?.filter((entry) =>
      filters.status
  ...
  }, [filters, initialState?.allBillableTimeEntries]);
  ...
  const data = useMemo(() => billableTimeEntriesData, [billableTimeEntriesData]);
  ...
  return (
    <div className={`main-content ${sidebarClose ? "sidebarClose" : ""}`}>
      ...
      <TimeEntriesFilter
        ...
        forTeamLeader={false}
        timeEntries={initialState?.allBillableTimeEntries}
      ...
      ) : billableTimeEntriesData?.length > 0 ? (
